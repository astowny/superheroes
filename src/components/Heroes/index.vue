<template>
	<div class="py-3 grey lighten-2">
		...
		<div v-if="selectedHero">
			<hero-detail :heroId="selectedHero.id" @deleted="selectedHero = null" @editing="handleEditing"  class="mb-2"/>
			<v-divider></v-divider>
		</div>
		<v-row class="fill-height mt-2" align-content="center" justify="center">
			<v-col v-if="!currentPageHeroes.length" class="text-center" cols="6">
				{{$t('loading')}}
				<v-progress-linear
					class="mx-auto"
		...
		>
			...
			<v-list-item-content :class="hero === selectedHero ? 'white--text' :''">
				...
				<v-list-item-subtitle v-html="hero.description" :class="hero === selectedHero ? 'white--text' :''"></v-list-item-subtitle>
			</v-list-item-content>
		</v-list-item>
		<!--  pagination  -->
